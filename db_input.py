...
class MetadataInputting:
    ...
    def get_or_create_alias(self, name: str) -> str:
        self.cursor.execute("SELECT id FROM aliases WHERE alias = ?",
                            (name,))
    ...
    def is_shared_alias(self, alias_id: str) -> bool:
        self.cursor.execute("SELECT shared_alias FROM aliases WHERE id = ?",
                            (alias_id,))
    ...
    def db_into_characters(self):
        for character in self.clean_info["characters"]:
            ...
            self.insert_comic_character(alias_id,
                                        character_id,
                                        certainity)
